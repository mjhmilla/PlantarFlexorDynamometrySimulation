...
nsim    = benchConfig.nSim;

assert(nsim == length(benchConfig.initialActivationState));
...
for i = 1:1:nsim
    ...
    if(benchConfig.numberOfMuscleStates ~= 0)     
        ...
        initSoln = calcInitialMuscleStateFcn(benchConfig.initialActivationState(1,i),...
                                          pathFcn(0),...                                          
                                          calcMuscleInfoFcn,...
                                          initConfig);
    ...
    activationState= ye(:,1);
    if(n >= 1)            
        muscleStateV = ye(:,2:1:(n+1));
    end

    workOfBoundary    = ye(:,n+2);
    workOfActiveFiber = ye(:,n+3);
    workOfDamping     = ye(:,n+4);
    ...
    for j=1:1:length(tV)
        ...
        mtInfo = calcMuscleInfoFcn(activationState(j,i),...                                   
                                   pathState,...
                                   muscleState);
    ...
    activationMax = max(benchRecord.activation(:,i));
    activationMax = min( activationMax, 1);
    activationMax = max( activationMax,0);
    
    clr01 = abs( benchConfig.color.*activationMax ...
              + [0.5,0.5,0.5].*(1-activationMax));
