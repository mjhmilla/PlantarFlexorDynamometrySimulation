...
if(exist([saveToFolder,'normMuscleCurves_',...
    ...
    if(flag_useFlatActiveForceLengthCurve==1)
      activeForceLengthCurve.ypts = ones(size(activeForceLengthCurve.ypts));
    end
    ...
    %Fiber Passive Force
    %%
    eZero = 0 + shiftFiberForceLengthCurve; 
    eIso  = 0.7 + shiftFiberForceLengthCurve;
    kLow  = 0.2;
    kIso  = 2/(eIso-eZero);
    curviness = 0.75;
    computeIntegral = 1;
...
%                                             computeIntegral, muscleName);
    ...
    flag_useOctave=0;
    smallNumericallyNonZeroNumber = 0;

    if(flag_enableNumericallyNonZeroGradients==1)
      smallNumericallyNonZeroNumber=1e-6;
    end
